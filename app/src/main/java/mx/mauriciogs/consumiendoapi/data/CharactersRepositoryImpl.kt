...
import mx.mauriciogs.consumiendoapi.data.model.toCharacter
...
class CharactersRepositoryImpl: CharacterRepository {
    ...
    override suspend fun getOneCharacter(id: Int): ResultState<mx.mauriciogs.consumiendoapi.domain.model.Character>{
        val result = try {
            val response = retrofitService.getOneCharacter(id).toCharacter()
            response
        }catch (e: Exception){
            println(e.printStackTrace())
            null
        ...
        return ResultState.Error("Ocurrio un error al hacer la peticion")
    }
